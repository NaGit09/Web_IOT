...
from app.PLC.plc_logo import plc_logo
...
class SocketMessage:
    ...
    def __init__(self, app, async_mode, cors_allowed_origins):
        self.socketio = SocketIO(
            app, async_mode=async_mode, cors_allowed_origins=cors_allowed_origins
        )
    ...
    def handle_toggle_relay(data):
        ...
        try:
            ...
            plc_logo.write_relay(relay_id, new_state)

            # 2. Cập nhật DB
            HandleDB().update_relay_state_db(relay_id, new_state)
        ...
        while True:
            try:
                ...
                for relay, state in converted.items():
                    HandleDB().update_relay_state_db(relay, state)
